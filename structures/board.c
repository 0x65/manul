...
void make_move(board_t* board, move_t move) {
    ...
    int from_square = MOVE_FROM(move),
        ...
        moving_piece = MOVING_PIECE(move),
        captured_piece = CAPTURED_PIECE(move),
    ...
    #define UPDATE_CASTLE_BITBOARDS(color, rook, to, from) \
        board->piece_bitboard[rook] ^= SQUARE_MASK[to] | SQUARE_MASK[from]; \
    ...
    #define MAKE_CAPTURE(other_color, square_offset) \
    ...
    if (board->to_move == WHITE) {
        ...
        if (IS_CASTLE(move)) {
            MAKE_CASTLE(WHITE, 0xC, H1, F1, A1, D1);
        }
        if (captured_piece) {
            MAKE_CAPTURE(BLACK, -8);
        }
        if (moving_piece == WHITE_PAWN) {
            MAKE_PAWN_MOVE(WHITE, 8);
        }

        if (moving_piece == WHITE_KING) {
            board->king_position[WHITE] = to_square;
        } else {
            board->piece_bitboard[moving_piece] ^= from_to_bitboard;
        }
        ...
        if (IS_CASTLE(move)) {
            MAKE_CASTLE(BLACK, 0x3, H8, F8, A8, D8);
        }
        if (captured_piece) {
            MAKE_CAPTURE(WHITE, 8);
        }
        if (moving_piece == BLACK_PAWN) {
            MAKE_PAWN_MOVE(BLACK, -8);
        }

        if (moving_piece == BLACK_KING) {
            board->king_position[BLACK] = to_square;
        } else {
            board->piece_bitboard[moving_piece] ^= from_to_bitboard;
        }
    ...
    #undef UPDATE_CASTLE_BITBOARDS
    #undef MAKE_CASTLE
    #undef MAKE_CAPTURE
    #undef MAKE_PAWN_MOVE
    ...
    if (board->can_castle != board->state[board->state_index].can_castle) {
        if ((board->can_castle & WHITE_CASTLE_OO) != (board->state[board->state_index].can_castle & WHITE_CASTLE_OO)) {
            board->hash_key ^= hash_keys.white_king_side_castle;
        }
        if ((board->can_castle & WHITE_CASTLE_OOO) != (board->state[board->state_index].can_castle & WHITE_CASTLE_OOO)) {
            board->hash_key ^= hash_keys.white_queen_side_castle;
        }
        if ((board->can_castle & BLACK_CASTLE_OO) != (board->state[board->state_index].can_castle & BLACK_CASTLE_OO)) {
            board->hash_key ^= hash_keys.black_king_side_castle;
        }
        if ((board->can_castle & BLACK_CASTLE_OOO) != (board->state[board->state_index].can_castle & BLACK_CASTLE_OOO)) {
...
void undo_move(board_t* board, move_t move) {
    ...
    #define UNDO_CASTLE(color, king_side_from, king_side_to, queen_side_from, queen_side_to) \
        if (to_square > from_square) { \
            UPDATE_CASTLE_BITBOARDS(color, color##_ROOK, king_side_from, king_side_to); \
        } else { \
            UPDATE_CASTLE_BITBOARDS(color, color##_ROOK, queen_side_from, queen_side_to); \
        }

    #define UNDO_CAPTURE(other_color, square_offset) \
        int captured_square = to_square; \
        if (IS_ENPASSANT(move)) { \
            captured_square += square_offset; \
        } \
    ...
    if (board->to_move == BLACK) {
        ...
        if (IS_CASTLE(move)) {
            UNDO_CASTLE(WHITE, H1, F1, A1, D1);
        }
        if (IS_PROMOTION(move)) {
            UNDO_PROMOTION(WHITE_PAWN);
        }
        if (captured_piece) {
            UNDO_CAPTURE(BLACK, -8);
        }

        if (moving_piece == WHITE_KING) {
            board->king_position[WHITE] = from_square;
        } else {
            board->piece_bitboard[moving_piece] ^= from_to_bitboard;
        }
        ...
        if (IS_CASTLE(move)) {
            UNDO_CASTLE(BLACK, H8, F8, A8, D8);
        }
        if (IS_PROMOTION(move)) {
            UNDO_PROMOTION(BLACK_PAWN);
        }
        if (captured_piece) {
            UNDO_CAPTURE(WHITE, 8);
        }

        if (moving_piece == BLACK_KING) {
            board->king_position[BLACK] = from_square;
        } else {
            board->piece_bitboard[moving_piece] ^= from_to_bitboard;
        }
...
void set_board_from_fen_string(board_t* board, const char* fen_string) {
    int fen_index = 0, fen_length = strlen(fen_string);
    char current_char;
    ...
    for (int rank = 7; rank >= 0; rank--) {
        ...
        do {
            if (file > 7) {
                fprintf(stderr, "FEN read error: too many files\n");
                return;
            }
    ...
    if (fen_string[fen_index-1] != ' ') {
        fprintf(stderr, "FEN read error: too many ranks\n");
        return;
    }

    // set side to move
    if (fen_string[fen_index] == 'W' || fen_string[fen_index] == 'w') {
        board->to_move = WHITE;
    } else if (fen_string[fen_index] == 'B' || fen_string[fen_index] == 'b') {
        board->to_move = BLACK;
        board->hash_key ^= hash_keys.side;
    } else {
        fprintf(stderr, "FEN read error: invalid side to move\n");
        return;
    }
    ...
    if (fen_index < fen_length) {
        do {
            switch (fen_string[fen_index]) {
                ...
                case '-': break;
                default:
                    fprintf(stderr, "FEN read error: invalid castle permission\n");
                    return;
            }
    ...
    if (fen_index+1 < fen_length && fen_string[fen_index] != '-') {
        ...
        if (board->passed_square < A1 || board->passed_square > H8) {
            fprintf(stderr, "FEN read error: invalid en-passant square\n");
            return;
        }
...
unsigned int repetition_count(const board_t* board) {
    ...
    for (int i = board->state_index - 2, last_i = board->state_index - board->fifty_move_rule_counter; i >= last_i; i -= 2) {
        if (board->state[i].hash_key == board->hash_key) {
            repetitions++;
        }
    }
