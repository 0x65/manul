...
move_t* generate_pseudolegal_moves(const board_t* board, move_t* move_list) {
    ...
    #define GENERATE_PAWN_MOVES(color, other_color, double_rank, promotion_rank) \
            ...
            while (existing_pieces) { \
                ...
                while (possible_moves) { \
                    ...
                    } else *move_list++ = (from_square) + (to_square << 6) + (color##_PAWN << 12) + (board->pieces[to_square] << 16); \
                    REMOVE_FIRST_BIT_SET(possible_moves); \
                } \
    ...
    if (board->to_move == WHITE) {
        ...
        if ((board->can_castle & WHITE_CASTLE_OO) && !(board->all_pieces_bitboard & 0x60)) *move_list++ = 0x20B184;
        if ((board->can_castle & WHITE_CASTLE_OOO) && !(board->all_pieces_bitboard & 0xE)) *move_list++ = 0x20B084;
        ...
        if ((board->can_castle & BLACK_CASTLE_OO) && !(board->all_pieces_bitboard & 0x6000000000000000)) *move_list++ = 0x20CFBC;
        if ((board->can_castle & BLACK_CASTLE_OOO) && !(board->all_pieces_bitboard & 0xE00000000000000)) *move_list++ = 0x20CEBC;
...
char is_king_in_check(const board_t* board, unsigned int side) {
    #define CHECK(color, other_color) int king_position = board->king_position[color]; \
    ...
    if (side == WHITE) { CHECK(WHITE, BLACK); }
    else { CHECK(BLACK, WHITE); }
    return 0;
}
...
bool is_theoretical_draw(const board_t* board) {
    ...
    if ((total_material < 13) && !board->piece_bitboard[WHITE_PAWN] && !board->piece_bitboard[BLACK_PAWN]) {
        ...
        if (total_material == 6) {
            ...
            if (board->material[BLACK] == 6 && !board->piece_bitboard[BLACK_BISHOP]) return true;
        } else if (total_material == 9 && board->piece_bitboard[WHITE_KNIGHT] && board->piece_bitboard[BLACK_KNIGHT]) return true;
        else if (total_material == 13 && board->piece_bitboard[WHITE_ROOK] && board->piece_bitboard[BLACK_ROOK]) return true;
    }
    return false;
}
...
bool is_legal_castle(board_t* board, move_t castle_move) {
    ...
    #define CASTLE_CHECK(color) if (is_king_in_check(board, color)) return false; \
        if (to_square < from_square) { \
            ...
            for (int i = from_square+1; i <= to_square; i++) { \
                test_move = from_square + (i<<6) + (color##_KING<<12); \
                make_move(board, test_move); \
                in_check = is_king_in_check(board, WHITE); \
                undo_move(board, test_move); \
                if (in_check) return false; \
            } \
        }

    if (board->to_move == WHITE) { CASTLE_CHECK(WHITE); }
    else { CASTLE_CHECK(BLACK); }
    return true;
}
...
unsigned int count_legal_moves(board_t* board) {
    ...
    for (int i = 0; &move_list[i] != end_of_move_list; i++) {
        if (IS_CASTLE(move_list[i]) && !is_legal_castle(board, move_list[i])) continue;
        make_move(board, move_list[i]);
        if (!is_king_in_check(board, !board->to_move)) moves_found++;
