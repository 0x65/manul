...
/*
    ...
    amount of moves for a set of given positions. Returns amount of processor ticks the test took
    to run.
*/
int test_generate_pseudolegal_moves() {
    int i;
    unsigned long number_of_generated_moves;
    ...
    assert(ELEMENTS_IN(fen_positions) == ELEMENTS_IN(number_of_correct_moves));
    ...
    for (i = 0; i < ELEMENTS_IN(fen_positions); i++) {
        ...
        number_of_generated_moves = end_of_list - move_list;
