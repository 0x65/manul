...
int search(board_t* board, int alpha, int beta, unsigned int ply, unsigned int depth) {
    ...
    if (repetition_count(board) >= 3) return DRAW_SCORE;
    ...
    for (int i = 0; &move_list[i] != end_of_move_list; i++) {
        ...
        if (is_king_in_check(board, !board->to_move)) undo_move(board, move_list[i]);
        else {
            moves_found++;
            ...
            } else new_score = -search(board, -beta, -alpha, ply+1, depth-1);

            undo_move(board, move_list[i]);

            if (new_score >= beta) {
                if (board->to_move == WHITE) white_history_heuristic[MOVE_FROM(move_list[i])][MOVE_TO(move_list[i])] += depth*depth;
                else black_history_heuristic[MOVE_FROM(move_list[i])][MOVE_TO(move_list[i])] += depth*depth;
                return beta;
            }
            if (new_score > alpha) {
                alpha = new_score;
    ...
    if (principal_variation_moves_found) {
        if (board->to_move == WHITE) white_history_heuristic[MOVE_FROM(principal_variation_moves[ply][ply])][MOVE_TO(principal_variation_moves[ply][ply])] += depth*depth;
        else black_history_heuristic[MOVE_FROM(principal_variation_moves[ply][ply])][MOVE_TO(principal_variation_moves[ply][ply])] += depth*depth;
    }

    if (!moves_found) {
        if (is_king_in_check(board, board->to_move)) return -CHECKMATE_SCORE+ply-1;
        else return DRAW_SCORE;
    }

    if (board->fifty_move_rule_counter >= 100) return DRAW_SCORE;
...
void order_moves(move_t* move_list, move_t* end_of_move_list, int current_index, bool to_move, unsigned int ply, unsigned int depth) {
    ...
    if (to_move == WHITE) { history_heuristic(white_history_heuristic); }
    else { history_heuristic(black_history_heuristic); }
}
